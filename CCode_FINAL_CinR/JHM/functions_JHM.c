...
double MCMC_K_p_JHM(struct_data_JHM *D,struct_para_JHM *D_para,struct_priors_JHM *D_priors,double para,int c,int l, int m){
  ...
  for (l=0;l<D->L;l++){
    ...
    SUM+=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(D_para->sigma_K_o)/D_priors->df)/**/+2*log(trun_const_low_JHM(0,exp(para),exp(D_para->sigma_K_o)));
  }
...
double MCMC_r_p_JHM(struct_data_JHM *D,struct_para_JHM *D_para,struct_priors_JHM *D_priors,double para,int c,int l, int m){
  ...
  for (l=0;l<D->L;l++){
    ...
    SUM+=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(D_para->sigma_r_o)/D_priors->df)/**/+2*log(trun_const_low_JHM(0,exp(para),exp(D_para->sigma_r_o)));
  }
...
double MCMC_sigma_K_o_JHM(struct_data_JHM *D,struct_para_JHM *D_para,struct_priors_JHM *D_priors,double para,int c,int l, int m){
  ...
  for (l=0;l<D->L;l++){
    ...
    SUM+=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(para)/D_priors->df)-(para)/**/+2*log(trun_const_low_JHM(0,exp(D_para->K_p),exp(para)));
  }
...
double MCMC_sigma_r_o_JHM(struct_data_JHM *D,struct_para_JHM *D_para,struct_priors_JHM *D_priors,double para,int c,int l, int m){
  ...
  for (l=0;l<D->L;l++){
    ...
    SUM+=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(para)/D_priors->df)-(para)/**/+2*log(trun_const_low_JHM(0,exp(D_para->r_p),exp(para))) ;
  }
