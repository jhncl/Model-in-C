...
double MCMC_A(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
  double density,F,SUM=0;
  int ll;
  for (c=0;c<2;c++){
  for (l=0;l<D->L;l++){
...
double MCMC_B(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
  double density,F,SUM=0;
  int ll;
  for (c=0;c<2;c++){
    for (l=0;l<D->L;l++){
...
/* printf("%g %g",D_priors->eta_nu_p,D_priors->eta_nu) ;print=3;*/
  ...
  for (j=0;j<thin;j++){
   ...
   D_MH->hK=0.1;*/
	...
	D_para->r_clm[mm]=MCMC_base_truncate_high(3.5,
