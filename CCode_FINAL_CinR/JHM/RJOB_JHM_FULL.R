...
model {
	for (l in 1:N){
		for (c in 1:2){
			...
			K_clm_L[(SHIFT[c]+NoSum[l,c]+m)] ~ dnorm(log(K_o_l[l])+(alpha_c[c]+(delta_l[l,c]*log(gamma_cl[l,c]))),exp(tau_K_cl[l,c]))T(,0)
         		r_clm[(SHIFT[c]+NoSum[l,c]+m)]<-exp(r_clm_L[(SHIFT[c]+NoSum[l,c]+m)])				           
    			r_clm_L[(SHIFT[c]+NoSum[l,c]+m)] ~ dnorm(log(r_o_l[l])+(beta_c[c]+(delta_l[l,c]*log(omega_cl[l,c]))),exp(tau_r_cl[l,c]))T(,3.5)
			}
