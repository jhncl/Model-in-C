...
double MCMC_K_clm(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	mm=D->NoSUM[ll]+m;
	nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N;
	F=(D->y[nn]-exp(D_para->P))/(D->x[nn]-0)-exp(D_para->r_clm[mm])*D->y[nn]*(exp(para)-D->y[nn]);
	SUM+=F*F*exp(D_para->nu_l[ll]);
	for (n=1;n<D->NoTIME[mm];n++){
		nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N+n;
		F=(D->y[nn]-D->y[nn-1])/(D->x[nn]-D->x[nn-1])-exp(D_para->r_clm[mm])*D->y[nn]*(exp(para)-D->y[nn]);
		SUM+=F*F*exp(D_para->nu_l[ll]);
	}	
...
double MCMC_r_clm(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	mm=D->NoSUM[ll]+m;
		nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N;
		F=(D->y[nn]-exp(D_para->P))/(D->x[nn]-0)-exp(para)*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
		SUM+=F*F*exp(D_para->nu_l[ll]);
		for (n=1;n<D->NoTIME[mm];n++){
			nn=l*D->M*D->N + m*D->N + n;
			F=(D->y[nn]-D->y[nn-1])/(D->x[nn]-D->x[nn-1])-exp(para)*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
			SUM+=F*F*exp(D_para->nu_l[ll]);
		}	
...
double MCMC_nu_l(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	int n,mm,nn,ll;
		...
		for (m=0;m<D->NoORF[ll];m++){
			mm=D->NoSUM[ll]+m;
			nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N;
				F=(D->y[nn]-exp(D_para->P))/(D->x[nn]-0)-exp(D_para->r_clm[mm])*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
				SUM+=-para+F*F*exp(para);
			for (n=1;n<D->NoTIME[mm];n++){
				nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N+n;
				F=(D->y[nn]-D->y[nn-1])/(D->x[nn]-D->x[nn-1])-exp(D_para->r_clm[mm])*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
				SUM+=-para+F*F*exp(para);
			}
		}
...
double MCMC_P(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	for (c=0;c<2;c++){
		...
		ll=c*D->L+l;
				...
				mm=D->NoSUM[ll]+m;
				nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N;
				F=(D->y[nn]-exp(para))/(D->x[nn]-0)-exp(D_para->r_clm[mm])*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
				SUM+=F*F*exp(D_para->nu_l[ll]);
				for (n=1;n<D->NoTIME[mm];n++){
					nn=c*D->SHIFTlmn+l*D->M*D->N + m*D->N+n;
					F=(D->y[nn]-D->y[nn-1])/(D->x[nn]-D->x[nn-1])-exp(D_para->r_clm[mm])*D->y[nn]*(exp(D_para->K_clm[mm])-D->y[nn]);
