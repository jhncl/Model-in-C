...
double MCMC_base_adaptive(gsl_rng *RNG, struct_data *D,struct_para *D_para,struct_priors *D_priors,double *accept,double *h,double para,double (*foo)(struct struct_data *D,struct struct_para *D_para,struct struct_priors *D_priors,double,int,int,int),int c,int l, int m){
...
double MCMC_Z_p(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
  ...
  for (l=0;l<D->L;l++){
    ...
    SUM+=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(D_para->sigma_Z)/D_priors->df);
  }
  F=D_para->Z_p-para;
...
double MCMC_Z_l(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
  ...
  F=para-D_para->Z_p;
  density=(1+D_priors->df)*gsl_sf_log(1+pow(F,2)*exp(D_para->sigma_Z)/D_priors->df)+SUM;
  return(-0.5*density); 
}
...
int gibbsandMHloop(int iter,int thin,gsl_rng *RNG,struct_data *D,struct_para *D_para,struct_priors *D_priors ,struct_MH *D_MH,int print,double *OUT,char **HEADER){
  int i,j,l,*T,t;
T=&t;
*T=0;
  ...
  */
  ...
  for (i=0;i<iter;i++){
    for (j=0;j<thin;j++){
      ...
      D_para->upsilon_c[1]=MCMC_base(RNG,D,D_para,D_priors,&D_MH->accept_up,&D_MH->hupsilon_c,D_para->upsilon_c[1],MCMC_upsilon_c,-999,-999,-999);
	...
	D_para->gamma_cl[l]=MCMC_base_truncate_low(0,
						   RNG,D,D_para,D_priors,&D_MH->accept_Z,&D_MH->hgamma_cl,exp(D_para->gamma_cl[l]),MCMC_gamma_cl,-999,l,-999);
