	...
	for (l=0;l<D->L;l++){
		ll=D->L+l;
 		for (m=0;m<D->NoORF[ll];m++){
			mm=D->NoSUM[ll]+m;
			F=exp( exp(para)*(D_para->Z_l[l]+D_para->delta[l]*D_para->gamma[l]) );
			SUM=SUM+F*(F-2*D->y[mm])*exp(D_para->nu_l[l])*exp(D_para->upsilon_c[1]);
		}
	}	
...
double MCMC_sigma_Z(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	double density,SUM=0;
	for (l=0;l<D->L;l++){
		SUM=SUM-para+(D_para->Z_l[l]-D_para->Z_p)*(D_para->Z_l[l]-D_para->Z_p)*exp(para);
	}	
	density=para*(para-2*D_priors->eta_Z_mu)*D_priors->psi_Z+SUM; 
	return(-0.5*density); 
}
...
double MCMC_upsilon_c(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	for (l=0;l<D->L;l++){
		...
		for (m=0;m<D->NoORF[ll];m++){
			mm=D->NoSUM[ll]+m;
			F=exp( exp(D_para->alpha[c])*(D_para->Z_l[l]+c*D_para->delta[l]*D_para->gamma[l]) );
			SUM=SUM-para+(D->y[mm]-F)*(D->y[mm]-F)*exp(para)*exp(D_para->nu_l[l]);
		}
	}
...
double MCMC_nu(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	for (c=0;c<2;c++){
		for (m=0;m<D->NoORF[c*D->L+l];m++){
			...
			F=exp( exp(D_para->alpha[c])*(D_para->Z_l[l]+c*D_para->delta[l]*D_para->gamma[l]) );
			SUM=SUM+(D->y[mm]-F)*(D->y[mm]-F)*exp(para)*exp(D_para->upsilon_c[c])-para;
		}
	}
...
double MCMC_gamma(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	for (m=0;m<D->NoORF[ll];m++){
		mm=D->NoSUM[ll]+m;
			F=exp( exp(D_para->alpha[1])*(D_para->Z_l[l]+D_para->delta[l]*para) );
			SUM=SUM+F*(F-2*D->y[mm])*exp(D_para->nu_l[l])*exp(D_para->upsilon_c[c]);
	}	
...
double MCMC_Z_l(struct_data *D,struct_para *D_para,struct_priors *D_priors,double para,int c,int l, int m){
	...
	for (c=0;c<2;c++){
		...
		for (m=0;m<D->NoORF[ll];m++){
			mm=D->NoSUM[ll]+m;
			F=exp( exp(D_para->alpha[c])*(para+D_para->delta[l]*D_para->gamma[l]) );
			SUM=SUM+F*(F-2*D->y[mm])*exp(D_para->nu_l[l])*exp(D_para->upsilon_c[c]);
		}	
	}
...
double aug_delta(gsl_rng *RNG,struct_data *D,struct_para *D_para,struct_priors *D_priors,int l){
	...
	int m,ll=D->L+l,mm;
        for (m=0;m<D->NoORF[ll];m++){
		mm=D->NoSUM[ll]+m;
		F=exp( exp(D_para->alpha[0])*(D_para->Z_l[l]) );
		precision=exp(D_para->nu_l[l])*exp(D_para->upsilon_c[0]);
		SUMa=SUMa*sqrt(precision)*exp(-0.5*precision*F*F);
	}	
...
SUMa=(1-D_priors->p)*SUMa;
        ...
        for (m=0;m<D->NoORF[ll];m++){
		mm=D->NoSUM[ll]+m;
		F=exp( exp(D_para->alpha[1])*(D_para->Z_l[l]+D_para->delta[l]*D_para->gamma[l]) );
		precision=exp(D_para->nu_l[l])*exp(D_para->upsilon_c[1]);
		SUMb=SUMb*sqrt(precision)*exp(-0.5*precision*F*F);
		}
...
int i,j,l;
	...
	for (i=0;i<iter;i++){
		for (j=0;j<thin;j++){
			...
			D_para->Z_p=gauss_sample(RNG,D,0,D->L,D_para->Z_l,exp(D_para->sigma_Z),D_priors->Z_mu,D_priors->eta_Z_mu);
